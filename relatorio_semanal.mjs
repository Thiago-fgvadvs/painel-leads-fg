...
async function main() {
  ...
  for (let i = 0; i < gAtivos.length; i += 20) {
    ...
    for (const g of lote) { const ms = byId[g.id].ms.sort((a, b) => Date.parse(a.timestamp) - Date.parse(b.timestamp)); let aw = null, lastCli = null; for (const m of ms) { const cli = m.origin !== "user" && !m.isFromMe && !m.isFromBot; if (cli) { if (aw == null) aw = Date.parse(m.timestamp); if (m.text) lastCli = m.text; } else if (m.origin === "user" && aw != null) { const w = Math.round((Date.parse(m.timestamp) - aw) / 6000) / 10; gturns.push({ min: w, nome: users[m.userId] || "—", grupo: g.name }); const n = users[m.userId] || "—"; (gPer[n] ??= []).push(w); aw = null; lastCli = null; } } if (aw != null && g.currentTicketId) gPend.push({ grupo: g.name, dia: diaMes(new Date(aw)), ts: aw, msg: (lastCli || "(mídia/sem texto)").slice(0, 80) }); }
  }
